...
import com.daasuu.mp4compose.filter.Overlay.OverlayInfo;
...
public class TextureRenderer {
    ...
    private OverlayInfo overlayInformation;
    ...
    public void drawFrame(SurfaceTexture st, int textureID, GlFramebufferObject srcFbo) {
        ...
        float[] scale = getScaleAspectFit((int) overlayInformation.getRotation(), overlayInformation.getWidth(), overlayInformation.getHeight(), fbo.getWidth(), fbo.getHeight());
        float hFlip = overlayInformation.isHorizonatalFlip() ? -1.0f : 1.0f;
        float vFlip = overlayInformation.isVerticalFilp() ? -1.0f : 1.0f;

        Matrix.scaleM(mMVPMatrix, 0, scale[0] * hFlip, -scale[1] * vFlip, 1);
        Matrix.rotateM(mMVPMatrix, 0, -(overlayInformation.getRotation() + overlayInformation.getCustomRotaton()), 0.f, 0.f, 1.f);
    ...
    public static float[] getScaleAspectFit(int angle, int widthIn, int heightIn, int widthOut, int heightOut) {
        ...
        //modifying aspectRatio Calculation.. here calculate width based on height
        //actual code was
        /**
        ...
        if (widthIn > heightIn) {
            ...
            float heightOutCalculated = (float) heightOut / aspectRatioIn;

            if (heightOutCalculated < widthOut) {
                scale[1] = heightOutCalculated / widthOut;
            } else {
                scale[0] = heightOut * aspectRatioIn / heightOut;
            }
        } else {
            float aspectRatioIn = (float) widthIn / (float) heightIn;
            float heightOutCalculated = (float) widthOut / aspectRatioIn;
            ...
            if (heightOutCalculated < heightOut) {
                scale[1] = heightOut / heightOutCalculated;
            } else {
                scale[0] = heightOut * aspectRatioIn / widthOut;
            }
    ...
    public void setFrameBufferObject(GlFramebufferObject fbo, OverlayInfo vInfo) {
        this.fbo = fbo;
        this.overlayInformation = vInfo;
        prepareRenderer();
    }
}
