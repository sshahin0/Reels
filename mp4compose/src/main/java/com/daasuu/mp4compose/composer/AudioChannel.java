...
class AudioChannel {
    ...
    void drainDecoderBufferAndQueue(final int bufferIndex, final long presentationTimeUs) {
        ...
        /*if(data!=null){
            ...
            ShortBuffer editedBuffer = ByteBuffer.allocate(shortBuffer.capacity()).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer();
            shortBuffer.remaining();
            shortBuffer.rewind();

            for (int i = 0; i < shortBuffer.remaining(); i += outputChannelCount) {
                for (int c = 0; c < outputChannelCount; c++) {
                    try {
                        // Process the sample
                        short sample = shortBuffer.get();
                        // Put processed sample into encoder's buffer
                        editedBuffer.put((short) (sample * .9f));
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
            }
            editedBuffer.remaining();
            editedBuffer.rewind();
            buffer.data = editedBuffer;
        }*/
    ...
    boolean feedEncoder(long timeoutUs) {
        ...
        ShortBuffer outBuffer = encoder.getInputBuffer(encoderInBuffIndex).asShortBuffer();
    ...
    private long remixAndMaybeFillOverflow(final AudioBuffer input,
        ...
        while (inBuff.hasRemaining()){
            short sample = inBuff.get();
            outBuff.put((short) (sample*.9));
        }
        ...
        if (inBuff.remaining() > outBuff.remaining()) {
            ...
            final long consumedDurationUs = sampleCountToDurationUs(inBuff.position(), inputSampleRate, inputChannelCount);
