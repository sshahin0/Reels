...
public class DecoderSurface implements SurfaceTexture.OnFrameAvailableListener {
    private static final String TAG = DecoderSurface.class.getName();
    ...
    public static long glContexThreadID;
    public static boolean runInternalProcess = true;
    ...
    private void setup() {
        ...
        com.daasuu.gpuv.egl.EglUtil.setupSampler(previewTexture.getTextureTarget(), GL_LINEAR, GL_NEAREST);

        GLES20.glBindTexture(GL_TEXTURE_2D, 0);

        glContexThreadID = Thread.currentThread().getId();
        Log.d("FrameBufferObject", "onSurfaceCreated: GL Thread id " + glContexThreadID);
        runInternalProcess = true;
    ...
    void completeParams() {
        ...
        Matrix.frustumM(ProjectMatrix, 0, -1f, 1f, -1, 1, 5, 7);
...
//        // but we should be able to get away with it here.
//
//        if(GlFilter.isModified){
//            filter.enableModifiedVertices();
//        }
//        filter.setup();
...
//        Matrix.frustumM(ProjectMatrix, 0, -aspectRatio, aspectRatio, -1, 1, 5, 7);
//        Matrix.setIdentityM(MMatrix, 0);
//
//        Log.d("simul12", width + " " + height);
//
//        if (filter != null) {
//            filter.setFrameSize(width, height);
//        }
