...
public class CropImageView extends ImageView {
    ...
    public void setAspectRatio(final int aspectRatioX, final int aspectRatioY) {
        ...
        requestLayout();
        try {
            cropViewSizeChanged.onCropViewSizeChanged(0, 0, mBitmapRect, mSnapRadius, "");
    ...
    public Bitmap getCroppedImage() {
        ...
        final double scaleX = matrixValues[Matrix.MSCALE_X];
        final double scaleY = matrixValues[Matrix.MSCALE_Y];
        final double transX = matrixValues[Matrix.MTRANS_X];
        final double transY = matrixValues[Matrix.MTRANS_Y];

        //Log.d("simul", "tx : " + transX + " ty : " + transY);

        // Ensure that the left and top edges are not outside of the ImageView bounds.
        final double bitmapLeft = (transX > 0) ? -transX : 0;
        final double bitmapTop = (transY > 0) ? -transY : 0;
        ...
        double cropX = (bitmapLeft + Edge.LEFT.getCoordinate()) / scaleX;
        double cropY = (bitmapTop + Edge.TOP.getCoordinate()) / scaleY;
        ...
        double cropWidth = Math.min(Edge.getWidth() / scaleX, originalBitmap.getWidth() - cropX);
        double cropHeight = Math.min(Edge.getHeight() / scaleY, originalBitmap.getHeight() - cropY);


        //Log.d("simul", "cx : " + cropX + " cy : " + cropY + " cw : " + cropWidth + " ch : " + cropHeight);
        //Log.d("simul", "l : " + Edge.LEFT.getCoordinate() + " t : " + Edge.TOP.getCoordinate() + " r : " + Edge.RIGHT.getCoordinate() + " b : " + Edge.BOTTOM.getCoordinate());
    ...
    public void cropImage(){
        ...
        // Calculate the top-left corner of the crop window relative to the ~original~ bitmap size.
         cropX = Math.round((bitmapLeft + Edge.LEFT.getCoordinate()) / scaleX);
         cropY = Math.round((bitmapTop + Edge.TOP.getCoordinate()) / scaleY);

        // Calculate the crop window size relative to the ~original~ bitmap size.
        // Make sure the right and bottom edges are not outside the ImageView bounds (this is just to address rounding discrepancies).
        cropWidth = Math.min(Math.round(Edge.getWidth() / scaleX), originalBitmap.getWidth() - cropX);
        cropHeight = Math.min(Math.round(Edge.getHeight() / scaleY), originalBitmap.getHeight() - cropY);


        Log.d("simul", "cx : " + cropX + " cy : " + cropY + " cw : " + (cropWidth + cropX) + " ch : " + (cropHeight + cropY));
        //Log.d("simul", "l : " + Edge.LEFT.getCoordinate() + " t : " + Edge.TOP.getCoordinate() + " r : " + Edge.RIGHT.getCoordinate() + " b : " + Edge.BOTTOM.getCoordinate());
    }
    ...
    private RectF getBitmapRect() {
       ...
       // Log.d("simul", getWidth() + " Width height " + getHeight());

        final float right = Math.min(left + drawableDisplayWidth, getWidth());
        final float bottom = Math.min(top + drawableDisplayHeight, getHeight());

        //Log.e("nayeem", "scalex:" + scaleX + "scaley:" + scaleY + "transx:" + transX + "tansy:" + transY + "vw:" + drawableIntrinsicWidth + "vh:" + drawableIntrinsicHeight);
        //Log.e("nayeem", "lrft:"+left +"bottom"+bottom+ "right:"+right+"top"+top);
    ...
    private void initCropWindow(@NonNull RectF bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            final float horizontalPadding = 0.1f * bitmapRect.width() * 0;
            final float verticalPadding = 0.1f * bitmapRect.height() * 0;
    ...
    private void drawDarkenedSurroundingArea(@NonNull final Canvas canvas) {
        ...
        canvas.drawRect(bitmapRect.left-1, bitmapRect.top-1, bitmapRect.right, top, mSurroundingAreaOverlayPaint);
    ...
    private void drawBorder(@NonNull Canvas canvas) {
            ...
            final float top = Edge.TOP.getCoordinate() + mBorderThickness;
            final float bottom = Edge.BOTTOM.getCoordinate() - mBorderThickness;
            canvas.drawRect(Edge.LEFT.getCoordinate(), top, Edge.RIGHT.getCoordinate(), bottom, mBorderPaint);
    }
    ...
    private void drawCorners(@NonNull Canvas canvas) {
        ...
        final float lateralOffset = - mBorderThickness*2;
        ...
        final float startOffset =  mBorderThickness/2;
     ...
     */
    private CropViewSizeChanged cropViewSizeChanged;
    ...
    public void onActionMove(float x, float y) {
        ...
        invalidate();
        cropViewSizeChanged.onCropViewSizeChanged(x, y, mBitmapRect, mSnapRadius, mPressedHandle.name());

        cropImage();
    }

    public interface CropViewSizeChanged {
        void onCropViewSizeChanged(float x, float y, RectF mBitmapRect, float mSnapRadius, String name);
    }

    public void setCropViewSizeChanged(CropViewSizeChanged cropViewSizeChanged) {
        this.cropViewSizeChanged = cropViewSizeChanged;
    }
